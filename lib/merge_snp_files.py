...
def update_header_dict(entire_h, df, in_type):
    ...
    gsgt_str = ", ".join(
        filter(None, list(set(gsgt_version)))
    )  # this line makes a list of the entries in
    ...
    new_header_info = {
        "GSGT Version": gsgt_str,
        "Processing Date": proc_date_str,
        "Content": content_str,
    }
    ...
    new_header_info.update(
        {
            "Num SNPs": str(num_snps),
            "Total SNPs": str(num_snps),
    ...
    if input_type == "AFFY":
        ...
        if any("_x" in n for n in all_columns) and any("_y" in m for m in all_columns):
            ...
            for each_x in x_list:
                ...
                compare_df = xy_df.merge(
                    xy_df,
                    on=["probeset_id", each_x, matching_y],
                    how="left",
                    indicator="Exist",
                )
                ...
                x_pos, y_pos = [
                    merged_df.columns.get_loc(each_x),
                    merged_df.columns.get_loc(matching_y),
                ]
                ...
                merged_final_df = merged_final_df.merge(
                    xy_df, on="probeset_id", how="outer"
                )
        ...
        final_df = merged_final_df.apply(
            lambda x: x.replace("---", "NoCall") if x.name in col_list else x
        )
