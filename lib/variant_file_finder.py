...
def var_match(
    ...
    if not matching_files:
        ...
        var_outfile.close()
        message = (
            "No variant file contains all user-specified variants. The closest panel is "
            + best_match_panel
            + ", missing "
            + str(otherwise_best_matches[best_match])
            + " variant(s). Problem variants have been written to the file "
            + bad_var_file
            + "\n"
        )
        exit(message)
