...
import os
import sys
import file_format_checker as fc
...
def generate_outfile_name(file, output_suffix, logfile):
    ...
    if os.path.exists(outfile_name):
        warning_text = "Output file " + outfile_name + " already exists - deleting output file"
...
def gen_long_output_col_names(out_type):
    ...
    if out_type == 'TOP':
        ...
        message = "Allele type is not Top, Forward, Design, or Plus. Remove before proceeding"
        exit(message)
...
def easy_long_conversion(file_df, out_1, out_2, outfile_name, header_dict, file, logfile):
    """
    ...
    for sample in sorted_unique_samples:
        ...
        sub_sample_df[sample] = sub_long_df[sub_long_df['Sample ID'] == sample][[pos1, pos2]].apply(
            lambda x: ''.join(x), axis=1)
        sub_sample_df.dropna(inplace=True)
        sub_sample_df_list.append(sub_sample_df)
    output_df = reduce(lambda left, right: pd.merge(left, right, on=['SNP Name'], how='outer'), sub_sample_df_list)
...
def long_as_matrix(file_df, file, logfile):
    ...
    for samples in unique_samples:
        ...
        new_matrix[samples] = sub_matrix[[in_1, in_2]].apply(lambda x: ''.join(x), axis=1)
        sub_matrix_2 = new_matrix[['SNP Name', samples]].copy()
        sub_matrix_2.dropna(inplace=True)
        #print(sub_matrix_2)
        matrix_df_list.append(sub_matrix_2)
    matrix_out = reduce(lambda left, right: pd.merge(left, right, on=['SNP Name'], how='outer'), matrix_df_list)
    if logfile is not None:
        logging = make_logs.simple_log(log_array, file, logfile)
...
def conversion(dataframe, var_dataframe, in_t, out_t):
    ...
    for tup in tuples:
        ...
        if first_col == '.' and second_col == '.':
            first_col = '-'
            second_col = '-'
        out_first_second.append([name, sample,
                                 first_col, second_col])  # Get the program to just print out the found values
...
def list_to_concat_df(output_type, results_list):
    ...
    for li in results_list:
        mini_df = pd.DataFrame(li, columns=['SNP Name', 'Sample ID', out_sample1, out_sample2])
        df_list_concat.append(mini_df)
    output_dataframe = pd.concat(df_list_concat, axis=0, ignore_index=True)
    return output_dataframe


def split_and_convert(df_for_conversion, var_df, matrix_type, output_type, can_we_thread, n_threads, file, logfile):
    """
    ...
    in_a_name, in_b_name, out_a_name, out_b_name = get_df_colnames(matrix_type, output_type)
    split_df_list = []
    #print(col0_names)
    for col0 in col0_names:
        name = df_for_conversion[['SNP Name', col0]].copy()
        name['{}'.format(in_a_name)], name['{}'.format(in_b_name)] = zip(*name[col0].apply(lambda y: list(y)))
    ...
    for out_type in vals:
        ...
        if can_we_thread is True:
            with cf.ProcessPoolExecutor(max_workers=n_threads) as executor:
                allele = {executor.submit(conversion, df, var_df, matrix_type, out_type): df
    ...
    output_df = pd.concat(results_array, sort=False, axis=1, levels=[['SNP Name', 'Sample ID']])
    ...
    if rd_rm_marker_list:
        ...
        message = "The following markers do not have information associated with the output type " + output_type + \
...
def reorder_converted_df(converted_df, col0_names, file_df, specified_out, output_type, file, logfile):
    """
    ...
    timestr = time.strftime("%H:%M:%S")
    logfile_text = timestr + " ..... Reorder to match SNP and sample order of input file"
    log_array = [logfile_text]
    ...
    if output_type == 'LONG':
        ...
        new_output_df['output'] = new_output_df[[pos1, pos2]].apply(lambda x: ''.join(x), axis=1)
        ...
        for col0 in col0_names:
            ...
            new_output_df2.rename({new_output_df2.columns[0]: 'SNP Name'}, axis=1, inplace=True)
        ...
        pre_final_output = reduce(lambda left, right: pd.merge(left, right, on=['SNP Name'], how='outer'), df_list)
...
def write_to_outfile(reordered_df, specified_out, affy_AB_df, header_dict, file, outfile_name, logfile):
    """
    ...
    if specified_out != 'AFFY-PLUS':  # if the file out is illumina
        ...
        reordered_df.to_csv(outfile_name, index=None, mode='a', header=col_list, sep='\t')
        ...
        reordered_df.rename(columns={reordered_df.columns[0]: 'probeset_id'}, inplace=True)
        merged_df = reduce(lambda left, right: pd.merge(left, right, on=['probeset_id'], how='outer'),
                           [reordered_df, affy_AB_df])[interleave([affy_AB_df, reordered_df])]
...
def convert_file(input_dir, file_list, file_type, specified_out, output_suffix, n_threads, is_verbose, conversion_dir,
                 summarize, assembly, tabular, species, plink):
    """
    ...
    species_assembly_correlation = errors.assembly_species_error(conversion_dir, assembly, species)
    if species_assembly_correlation:
        pass
    else:
        exit("The assembly " + assembly + " is incompatible with species " + species + ". See README for acceptable "
                                                                                       "assemblies.")
    ...
    for file in snp_files_array:
        ...
        file_df, affy_AB_dict, header_row, header_dict, logfile2 = read_input_file_to_df(file_path, file, file_type, logfile1)
        ...
        var_list, logfile2_text, alt_bool = vff.var_match(variant_files, conversion_dir, file_df, file, converting_file, assembly, species)
        ...
        get_snp_pan = False  ######## CHANGE THIS IF YOU WANT TO PUT SNP PANEL OPTION IN HERE
        internal_sum = False
        format_check_plink = False
        determined_ft, correct_file, logfile3 = fc.file_format_check(input_dir, file, file_type,
                                                                      get_snp_pan, is_verbose, conversion_dir,
                                                                      logfile2, assembly, internal_sum, tabular, species, format_check_plink)
        file_is_correct, logfile4 = format_check_parse_errors(correct_file, determined_ft, file_type, file, logfile3)
        ...
        if determined_ft == 'LONG':
            ...
            if out_1 in file_df.columns:
                long_easy_output, logfile5 = easy_long_conversion(file_df, out_1, out_2, outfile_name, header_dict, file, logfile4)  # need to test whether the output file is correctly formatted
                df_for_conversion = None
                matrix_type = None
                logfile8 = logfile5
            else:
                # Convert the long file to a matrix-like array for conversion
                df_for_conversion, matrix_type, logfile5 = long_as_matrix(file_df, file, logfile4)
        else:
        ...
        if df_for_conversion is not None:
            converted_df, column_names, logfile6 = split_and_convert(df_for_conversion, var_df, matrix_type, output_type, can_we_thread, n_threads, file, logfile4)
            # Reorder converted dataframe to match the original order of samples and snp names
            reordered_df, logfile7 = reorder_converted_df(converted_df, column_names, file_df, specified_out, output_type, file, logfile6)
            # Write files reordered_df to output file
            matrix_or_long_hard_output, logfile8 = write_to_outfile(reordered_df, specified_out, affy_AB_dict, header_dict, file, outfile_name, logfile7)
        else:
            logfile8 = logfile4
        ...
        new_input_dir = '.'
        get_snp_panel = False
        correct_output_type, correct_output, logfile9 = fc.file_format_check(new_input_dir, outfile_name,
                                                                               specified_out, get_snp_panel,
                                                                               is_verbose, conversion_dir,
                                                                               logfile8, assembly, summarize, tabular, species, plink)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Performs file conversion between Illumina matrix and long formats")
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    out = convert_file(in_dir, f_list, f_type, o_type, o_suffix, threads, vb, conv_dir, args.summary, assembly_val, args.tabular, args.species, args.plink)
