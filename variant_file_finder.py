...
def get_var_df(var_dir, var_file, assembly, species, alt_bool):
    ...
    ref_df = whole_var_df[whole_var_df["VCF"] == "REF"].copy()
    alt_df = whole_var_df[whole_var_df["VCF"] == "ALT"].copy()
    merged_alt_ref = pd.merge(ref_df, alt_df, on=["marker_name", "alt_marker_name"])
    if alt_bool is True:
        merged_alt_ref.drop(columns=["marker_name"], inplace=True)
        merged_alt_ref.rename(columns={"alt_marker_name": "marker_name"}, inplace=True)
    else:
        pass
    merged_alt_ref.rename(columns={"marker_name": "Name", "DESIGN_x": "DESIGN_A", "DESIGN_y": "DESIGN_B",
                                   "TOP_x": "TOP_A", "TOP_y": "TOP_B", "PLUS_x": "PLUS_A", "PLUS_y": "PLUS_B",
                                   "FORWARD_x": "FORWARD_A", "FORWARD_y": "FORWARD_B"}, inplace=True)
    if alt_bool is True:
        merged_alt_ref_dropped = merged_alt_ref.drop(columns=["AB_x", "AB_y", "VCF_x", "VCF_y"])
    else:
        merged_alt_ref_dropped = merged_alt_ref.drop(columns=["alt_marker_name", "AB_x", "AB_y", "VCF_x", "VCF_y"])
    merged_alt_ref_dropped["SNP"] = ''
    merged_alt_ref_dropped["BLAST_strand"] = ''
    merged_alt_ref_dropped["Reference_allele_forward_strand"] = ''
    # Add positional info
    if alt_bool is True:
    ...
    merged_positional = pd.merge(merged_alt_ref_dropped, position_marker_info, on="Name")
