...
def get_compression_type(path):
    with open(path, "r", errors="ignore") as file_path:
...
def uncompressing(path):
    ...
    if filetype == "no match" or filetype == "native_uncompressed":
        ...
        with open(path, "r") as uncompressed_handle:
            return vff.parse_vars(uncompressed_handle)
    elif filetype == "gzip":
        with gzip.open(path, "rt") as gzipped_handle:
            return vff.parse_vars(gzipped_handle)
    elif filetype == 'bz2':
        with bz2.open(path, "rt") as bzipped_handle:
            return vff.parse_vars(bzipped_handle)
    elif filetype == "zip":
        with ZipFile(path, mode='r', allowZip64=True) as zipped_file:
            ...
            if len(zip_list) != 1:
                ...
                with zipped_file.open(zip_list[0], mode='r') as zipped_handle:
                    ...
                    return vff.parse_vars(items_file)
    else:
        pass
