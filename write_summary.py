...
def get_col_widths(sample):
    ...
    if len(sample[next(iter(sample))]) == 9:  # finds the length of the value (list) for the first entry in the dict
        is_long = True
    else:
        is_long = False
    total_cols = ['tot_snp', 'snp_gen', 'no_gen', 'hom', 'het', 'indel', 'incon', 'equiv', 'inequiv']
    if is_long:
        columns = total_cols
    else:
    ...
    length_dict.update({'max_sample': sample_len, 'max_tot_snp': max_tot_snp, 'max_snp_gen': max_snp_gen,
                        'max_hom': max_hom, 'max_het': max_het, 'max_indels': max_indels,
                        'max_consistent': max_consistent, 'max_inconsistent': max_inconsistent})
    ...
    head_dict.update({'max_sample': 6, 'max_tot_snp': 13, 'max_snp_gen': 24,
                        'max_hom': 13, 'max_het': 15, 'max_indels': 9,
                        'max_consistent': 23, 'max_inconsistent': 25})
    if is_long:
    ...
    sorted_comp_df['max_values'] = sorted_comp_df[['sample_lengths', 'header_lengths']].max(axis=1)
...
def write_summary(input_df, file_type, f_name, inconsistency_val, equiv_cols, inequiv_cols, tab_format):
    sample_dict = {}
    if file_type is None:
        ...
        if file_type != 'LONG':
            ...
            for member in samples:
                none_list = ['--', '---']
                number = input_df[~input_df[member].isin(none_list)].shape[0]  # make sure this works with affy data
                ...
                sample_dict.update({member: [total_snps, number, num_dashes, num_homozygous, num_heterozygous,
                                             num_indels, inconsistency_val, equiv_cols, inequiv_cols]})
            ...
            for each in samples:
                ...
                sub_df_no_gc = sub_df[['Sample ID', 'Allele1 - Top', 'Allele2 - Top', 'Allele1 - Forward',
                                       'Allele2 - Forward', 'Allele1 - AB', 'Allele2 - AB', 'Allele1 - Design',
                                       'Allele2 - Design', 'Allele1 - Plus', 'Allele2 - Plus']].copy()
                sub_df_no_gc.drop(['Sample ID'], axis=1, inplace=True)
                top_df = sub_df_no_gc[['Allele1 - Top', 'Allele2 - Top']].copy()
                top_df['Combine'] = top_df[['Allele1 - Top', 'Allele2 - Top']].apply(lambda x: ''.join(x), axis=1)
                nn_list = ['AA', 'TT', 'GG', 'CC']
                ...
                num_heterozygous = number - num_homozygous - num_indels
                # Total_snps, snps_with_data, snps_without_data, homozygous, heterozygous, indels
                sample_dict.update({each: [total_snps, number, not_present, num_homozygous, num_heterozygous,
                                           num_indels, inconsistency_val, equiv_cols, inequiv_cols]})
        ...
        with open(outfile_name, 'w') as summary:
            ...
            if tab_format is True:
                summary.write("Sample\tTotal SNPs\tSNPs with genotypes\tHomozygous\tHeterozygous\tIndels\tConsistent genotypes\tInconsistent genotypes")
                if file_type != 'LONG':
                    summary.write("\n")
                else:
                ...
                summary_line = "{sample:{w1}}{tot_snp:{w2}}{snp_gen:{w3}}{hom:{w4}}{het:{w5}}{ind:{w6}}{con:{w7}}{incon:{w8}}".\
                    format(sample="Sample", tot_snp="Total SNPs", snp_gen="SNPs with genotypes", hom="Homozygous",
                           het="Heterozygous", ind="Indels", con="Consistent genotypes", incon="Inconsistent genotypes",
                           w1=col_widths[0], w2=col_widths[1], w3=col_widths[2], w4=col_widths[3], w5=col_widths[4],
                           w6=col_widths[5], w7=col_widths[6], w8=col_widths[7])
                summary.write(summary_line)
                if file_type != 'LONG':
                    summary.write("\n")
                else:
                    summary_line = "{equiv:{w9}}{inequiv:{w10}}".format(equiv="Equivalent genotypes",
                                                                        inequiv="Inequivalent genotypes",
                                                                        w9=col_widths[8], w10=col_widths[9])
                    summary.write(summary_line + "\n")

            else:
                pass
            for items in sample_dict:
                percent_snps_with_data = str(sample_dict[items][1]) + " (" + str(round((sample_dict[items][1]/sample_dict[items][0])*100, 2)) + ")"
                percent_same_allele = str(sample_dict[items][3]) + " (" + str(round((sample_dict[items][3]/sample_dict[items][1])*100, 2)) + ")"
                percent_different_alelle = str(sample_dict[items][4]) + " (" + str(round((sample_dict[items][4]/sample_dict[items][1])*100, 2)) + ")"
                percent_indels = str(sample_dict[items][5]) + " (" + str(round((sample_dict[items][5]/sample_dict[items][1])*100, 2)) + ")"
                ...
                if file_type != 'LONG':
                    if tab_format:
                        output_line = "{sample}\t{tot_snp}\t{snp_gen}\t{hom}\t{het}\t{ind}\t{con}\t{incon}\n".\
                            format(sample=items, tot_snp=sample_dict[items][0], snp_gen=percent_snps_with_data,
                                   hom=percent_same_allele, het=percent_different_alelle, ind=percent_indels,
                                   con=consistent_genotypes, incon=inconsistent_genotypes)
                    else:
                        output_line = "{sample:<{w1}}{tot_snp:<{w2}}{snp_gen:<{w3}}{hom:<{w4}}" \
                                      "{het:<{w5}}{ind:<{w6}}{con:<{w7}}{incon:<{w8}}\n". \
                            format(sample=items, tot_snp=sample_dict[items][0], snp_gen=percent_snps_with_data,
                                   hom=percent_same_allele, het=percent_different_alelle, ind=percent_indels,
                                   con=consistent_genotypes, incon=inconsistent_genotypes, w1=col_widths[0],
                                   w2=col_widths[1], w3=col_widths[2], w4=col_widths[3], w5=col_widths[4],
                                   w6=col_widths[5], w7=col_widths[6], w8=col_widths[7])

                else:  # if file type IS Long
                    if tab_format:
                        output_line = "{sample}\t{tot_snp}\t{snp_gen}\t{hom}\t{het}\t{ind}\t{con}\t{incon}\t{equiv}\t" \
                                      "{inequiv}\n".format(sample=items, tot_snp=sample_dict[items][0],
                                                           snp_gen=percent_snps_with_data, hom=percent_same_allele,
                                                           het=percent_different_alelle, ind=percent_indels,
                                                           con=consistent_genotypes, incon=inconsistent_genotypes,
                                                           equiv=equiv_cols, inequiv=inequiv_cols)
                    else:
                        output_line = "{sample:<{w1}}{tot_snp:<{w2}}{snp_gen:<{w3}}{hom:<{w4}}" \
                                      "{het:<{w5}}{ind:<{w6}}{con:<{w7}}{incon:<{w8}}{equiv:<{w9}}{inequiv:<{w10}}\n".\
                            format(sample=items, tot_snp=sample_dict[items][0], snp_gen=percent_snps_with_data,
                                   hom=percent_same_allele, het=percent_different_alelle, ind=percent_indels,
                                   con=consistent_genotypes, incon=inconsistent_genotypes, equiv=equiv_cols,
                                   inequiv=inequiv_cols, w1=col_widths[0], w2=col_widths[1], w3=col_widths[2],
                                   w4=col_widths[3], w5=col_widths[4], w6=col_widths[5], w7=col_widths[6],
                                   w8=col_widths[7], w9=col_widths[8], w10=col_widths[9])
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Writes summary of matrix and long format files - "
    ...
    with open(args.input, 'r') as input_file:
        ...
        inequivalent = 0
        equivalent = 0
        write_summary(df, args.input_format, args.input, inconsistency, equivalent, inequivalent, args.t)
