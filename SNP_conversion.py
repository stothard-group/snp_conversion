...
parent_parser = argparse.ArgumentParser(add_help=False, description="Converts between Illumina matrix and long formats")
parser = argparse.ArgumentParser(add_help=True, description="Converts between Illumina matrix and long formats. To see "
...
sub_c = subs.add_parser(
    'merge_files',
    parents=[parent_parser],
    help="Merges files of the same format"
)
...
if args.command == 'convert_file':
    ...
    output = convert.convert_file(input_dir, args.file_list, args.input_format, args.output_format, args.output_name, args.threads, args.verbose_logging, args.key_dir, args.summary, args.assembly, args.tabular, args.species, args.plink)
elif args.command == 'check_format':
    # Send the appropriate commands to the format module
    log_file = None
    file_type, correct_format, return_log = ffc.file_format_check(input_dir, args.file_list, args.input_format, args.get_snp_panel, args.verbose_logging, args.key_dir, log_file, args.assembly, args.summary, args.tabular, args.species, args.plink)
    ...
    if not args.input_format:
        ...
        if args.input_format == 'affymetrix':
            output = merge.merge_affy_files(input_dir, args.file_list, args.input_format, args.output_name)
        else:
            output = merge.merge_matrix_files(input_dir, args.file_list, args.input_format, args.output_name)
