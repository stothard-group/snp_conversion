...
def retrieve_user_input_file(panel_path):
    ...
    """
    print("starting retrieve user input file")
    ...
    print("ending retrieve user input files")
...
def retrieve_all_variant_files(variant_dir, assembly, species):
    ...
    """
    print("starting retrieve variant files")
    ...
    if variant_exclude:
        ...
        pass
    print("ending retrieve variant files")
...
def long_file_to_array(illumina_df, logfile, file_name):
    ...
    """
    print("start long file to array")
    log_array = []
    type_list = ['PLUS']
    df_dict = {}
    for ty in type_list:
...
def check_input_snp_panel(snp_panel, file_name, file_type, variant_files, assembly, variant_directory,
    ...
    logfile_text = timestr + " ..... Checking the SNP panel"
    ...
    log_array.append(logfile_text)
    print("Finding the matching variant position file ")
    ...
    if affy_flag is True:
        matrix_type = 'FWD'
        converted_df, column_names, logfile = fc.split_and_convert(panel_dataframe, var_df, matrix_type, 'FWD', can_we_thread, n_threads, file_name, logfile)
        reordered_df, logfile = fc.reorder_converted_df(converted_df, column_names, panel_dataframe, 'FWD',
                                                        'FWD', file_name, logfile)
...
def subsampling_panel_and_varframe(panel_dataframe, variant_sub_dataframe, sample):
    ...
    pos_df_list = [pos_df_nullvals_removed, pos_df_nullvals_only]
...
def get_snp_panel_positional_info(panel_dataframe, var_dataframe, can_we_thread, n_threads, logfile, file_name):
    ...
    """
    print("start getting positional info")
    log_array = []
    timestr = time.strftime("%H:%M:%S")
...
#        positional_info_dict.update({each_sample: [pos_df_nullvals_removed, pos_df_nullvals_only]})
    print("finished getting positional info")
    if logfile is not None:
        logging = make_logs.simple_log(log_array, file_name, logfile)
...
def find_missing_samples(line, samples_strip):
    ...
    print("samples: " + ", ".join(samples_strip))
    print("vcf samples:" + ", ".join(vcf_samples))
    missing_samples = []
    kept_samples = []
...
def numeric_to_allele_conversion(geno_parsed_df):
    ...
    """
    print("starting allele conversion")
    print(geno_parsed_df.head(10))
...
def get_genotypes_from_vcf(genotype_path, subsampled_gt_df, full_gt_df, can_we_thread, n_threads, logfile, file_name):
    ...
    print("start getting genotypes from vcf")
    log_array = []
    timestr = time.strftime("%H:%M:%S")
...
def subsample_vcf_by_position(genotype_file_input, dict_of_panels, samples, filter_conditions, qual_crit, filter_crit, logfile, file_name):
    ...
    with open(genotype_file_input, 'r') as gt_file:
        for line in gt_file:
            if line.startswith("##"):
                ...
                print(missing_samples, kept_samples)
                break
            else:
    ...
    vcf_to_df = pd.read_csv(genotype_file_input, skiprows=vcf_header_count, sep='\t', usecols=columns_to_use,
                            dtype={'#CHROM': object})
    ...
    sample_panel.to_csv("check_sample_panel.txt", sep='\t')
...
def parse_reversals(neither_df, animal, snp_panel):
    """
    This determines if there are alleles where the order is flipped, but otherwise are identical
    :param neither_df: merged panel and vcf dataframe where there are no matches
    :param animal: the panel name of the animal in question (animal.1)
    :param snp_panel: df containing the panel snps
    :return: dataframe containing the new matches (with reversals)
    :return: dataframe containing no matches following processing
    """
    print("attempting to find reversals")
    # make new df column that has reversed vcf allele
    neither_df['panel_reversal'] = neither_df.loc[:, animal].apply(lambda x: x[::-1])
    # Check for matches again with the snp panel
    merged_neither_panel_and_vcf = pd.merge(left=neither_df, right=snp_panel, how='left',
                                            left_on=['BLAST_chromosome', 'BLAST_position', animal],
                                            right_on=['#CHROM', 'POS', 'vcf_reversal'], indicator='reverse_exist')
    merged_neither_panel_and_vcf['reverse_Both'] = np.where(merged_neither_panel_and_vcf.reverse_exist == 'both', 1, 0)
    print(merged_neither_panel_and_vcf)
...
def get_discordant_name_list(both_list, forward_df, reverse_df):
    ...
    print("discordant list len:")
    print(str(len(discordant_list)))
...
def comparison(animal, SNP_panel_dict, vcf_converted, filter_bool, qual_tofilter, filt_tofilter, log_array, logfile,
    ...
    merged_snp_vcf_panel = pd.merge(left=working_vcf, right=working_snp_panel,
                                    ...
                                    how='outer', indicator=True)
    print("merged snp vcf panel")
    print(merged_snp_vcf_panel)
    ...
    print("homozygous")
    print(correct_homozygous_df)
    print("correct heterozygous")
    print(correct_heterozygous_df)
    print("reverse heterozygous")
    print(reverse_heterozygous_df)
...
def count_ref_or_alt(hom_df, animal_snp_name):
    ...
    hom_df.to_csv('homozygous_df_affy_test.txt', sep='\t')
...
def generate_animal_stats(animal_basename, homozygous, het_forward, het_reverse, dashes, discordant, log_array, logfile,
    ...
    """
    print("starting stats for animal " + animal_basename)
    ...
    if total_homozygous != (homozygous_ref_informational_number + homozygous_alt_informational_number):
        print(total_homozygous, homozygous_ref_informational_number, homozygous_alt_informational_number)
